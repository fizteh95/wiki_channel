...
class ArticleFlow:
    def __init__(self, region: RegionPublic, uow: UnitOfWork, sender: AbstractSender, scraper: type[ArticleScraper]) -> None:
        self.region = region
        self.uow = uow
        self.sender = sender
        self.scraper = scraper

    async def send_article_day(self, last_article_day: ArticleDay, new_article: ArticleDay) -> None:
        """
        ...
        if (now.hour >= self.region.utc_hour_to_send_day and now.minute >= self.region.utc_minute_to_send_day) and (
            ...
            async with self.uow.atomic() as repo:
                await self.sender.send_article(article=new_article)
                await repo.create_article_of_day(article=new_article)

    async def send_article_good(self, last_article_good: ArticleGood, new_article: ArticleGood) -> None:
        now = datetime.datetime.now(datetime.UTC)
        if (now - last_article_good.send_time).seconds // 60 >= self.region.interval_for_goof_article:
            async with self.uow.atomic() as repo:
                await self.sender.send_article(article=new_article)
                await repo.create_good_article(article=new_article)
    ...
    async def article_day_scheduled(self) -> None:
        ...
        await self.send_article_day(last_article_day=last_article_day, new_article=new_article_day)
    ...
    async def article_good_scheduled(self) -> None:
        ...
        await self.send_article_good(last_article_good=last_article_good, new_article=new_article_good)
